...
Every certification & training whose latest completion has *already* lapsed — the inverse of Expiring Soon. Records are bucketed by how long ago they expired (≤ 1 month, 1–3, 3–6, 6–12, > 12 months) with a stacked bar by training type (click a band to filter the table) and an "Expired by Theatre" chart. The **Lapsed** dropdown narrows the whole report to a recent window — **Lapsed Any Time** (the default, everything ever), or lapsed within the last **3 / 6 / 12 months** — so you can focus on renewals still worth chasing instead of years of history. The window applies to the charts, KPI cards, subtotals, table and exports alike, and its edges line up exactly with the chart bands (pick 3 months and the "1–3 months" band is the last populated one, summing to the total). If you have clicked a band that falls outside the window you choose, the band filter clears itself rather than leaving an empty report. Group by theatre/region/country, search by name/email, and export to CSV/Excel/PDF. Retired (legacy) certs are shown by default; tick **Exclude retired (legacy) certs** to hide them. The report is computed on the server and the detail table is **paginated** — page through the results with the controls beneath the table, while the charts, KPI cards and exports always reflect the full filtered set. Also available as a scheduled export.
...
- **Moving the catalogue between systems** — An entry still in the **"needs attention"** list is exported with its **Type**, **Product** and **Function** left blank, because it has not been classified yet. On import, a row whose Type (or Parent Training Title), Product and Function cells are all filled in **completes** a matching entry that is still awaiting review, so it leaves the "needs attention" list. A blank cell means "not set": it never overwrites a value the receiving system already has, and a brand-new entry arriving with a blank cell is placed in "needs attention" rather than being given a guessed classification. The *Default Values for Unmapped Fields* apply only to columns the file does not have at all, and never mark an entry as reviewed. The import results say how many entries the file took out of "needs attention" (they are counted within **Updated**).
...
You can also **Ignore** an entry that simply isn't needed, from the "needs attention" list or from the Full Title page (where **Restore** brings it back). An ignored entry is left out of the dashboard, every report, scheduled exports and the public API, is no longer offered when configuring program and offering requirements, and is marked with a grey **Ignored** badge. Its completions are kept, so a learner's own record still shows the training, and nothing is deleted.
...
The **Renewal window** selector (Off / 1 / 3 / 6 / 12 months) projects compliance forward, using the same treatment as the program dashboards' *Compliance as of*:

- Attained figures read **current → projected** (e.g. `16 → 3`) with a **▼N expiring** note.
- A requirement or specialisation that is met **today** but falls below target by the end of the window is shaded **amber** and badged **"At risk in Nmo"** next to its green **Achieved** badge — it is compliant now and will not be then. An at-risk specialisation is expanded by default so the numbers behind it are visible.
- The target headline says so too, rather than reporting "fully compliant — nobody left to certify".
