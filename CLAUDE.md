...
- **ImportAlias** — PK: `id` (auto-increment). Fields: targetField (one of the `IMPORT_TARGET_FIELDS` keys in `lib/import-target-fields.ts` — `fullName`/`firstName`/`lastName`/`email`/`theatre`/`country`/`title`/`completedDate`/`company`), alias (raw header text), createdAt. Unique on (targetField, alias). Drives the `/admin/import` wizard's column auto-mapper: at mount it fetches `GET /api/import-aliases` and matches each header against the alias list (case/punctuation-insensitive). Defaults are seeded by the `20260623000000_import_aliases` migration; managed at `/admin/system-settings` → **Import Aliases** tab via `GET/POST /api/admin/import-aliases` and `PATCH/DELETE /api/admin/import-aliases/[id]` (SuperAdmin-only). Included in backup archives as `import_aliases.json` and wiped by both `wipe { scope: "data" | "all" }` modes.
...
- Backup archives are encrypted at rest when `ENCRYPTION_KEY` is configured: `generateBackupArchive()` wraps the JSZip output with `encryptBuffer()` from `lib/crypto.ts` (4-byte magic `TT01` + IV + GCM tag + ciphertext) and the file is saved as `*.zip.enc`. `loadBackupArchive(buf, passphrase?)` detects the magic and decrypts before handing bytes to JSZip, so older `*.zip` archives still restore. **Portable backups** (`POST /api/admin/backup/portable` with `{ passphrase }`, surfaced as the "Portable backup…" button on `/admin/backup`) use `encryptBufferWithPassphrase` (magic `TT02` + salt + IV + tag + ciphertext, scrypt-derived key) so they restore on a *different* system by re-entering the passphrase rather than matching `ENCRYPTION_KEY`; the restore upload form passes an optional `passphrase` to `loadBackupArchive`. Backups already exclude `passwordHash` and `mfaSecret`; the encryption protects the remaining PII (Students, TrainingTaken, Companies). The archive also includes `product_types.json` (restored before `training_data` so the `productTypeId` FK resolves); `lib/product-types.ts:prepareBackupRestore` reconstructs product types from older pre-migration archives that stored a `productType` enum string.
