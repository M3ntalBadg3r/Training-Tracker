...
src/
  ...
  lib/
    ...
    api-key.ts    # Public-API key utilities: generateApiKey (`tt_live_<base64url>`), hashApiKey (SHA-256), requireApiKey (Bearer/X-API-Key → AuthError on miss; throttled lastUsedAt/lastUsedIp audit), resolveApiKeyCompanyFilter (intersect key's companies with `?companyId=`), checkApiKeyRateLimit (120/min per key via rate-limit.ts)
    public-api.ts # authorizePublicRequest(request): the shared guard chain for `/api/public/v1/*` — runs requireApiKey, the per-key rate limit (429), and company-filter resolution; returns either `{ companyIds }` or a ready NextResponse
