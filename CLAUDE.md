...
- **Day/month-swapped native Excel dates**: a `DD/MM`-locale Excel round-trip can silently transpose the day and month of native date serials. `/admin/import` detects the swap signature client-side — the mapped completed-date column has ≥ 5 native serials and **every** decoded day-of-month is ≤ 12 (a genuine column reaches 31) — and prompts the user via a confirm modal (mirrors the format-mismatch modal). On confirm, native serials are un-swapped via `swapMonthDayIso(iso)` from `lib/date-format.ts` (transposes month/day, returns null when day > 12); text cells are untouched and still flow through `detectFormat`. Gated by `unswapExcelDates`/`swapAcknowledged` state and threaded through `resolveDateCell(raw, unswap)`. Client-side only; the server is unchanged.
...
- **Import column mapping**: The import API auto-maps columns with fuzzy matching and supports type aliases (e.g., `ilt` → `InstructorLedTraining`, `pre-sales` → `PreSales`). The name can come from a single `Full Name` column **or** separate `First Name` + `Last Name` columns (merged into `Student.fullName` per-row in `mappedRows`); the wizard requires Full Name OR both parts. An explicit Full Name value wins per-row, then First+Last, then a name derived from the email local part.
