...
- Security headers HSTS, X-Frame-Options, X-Content-Type-Options, Referrer-Policy, Permissions-Policy are set in `next.config.ts`. **Content-Security-Policy is set per-request from `src/proxy.ts`** with a fresh nonce; Next.js stamps the nonce onto its own hydration `<script>` tags via the `x-nonce` request header. CSP uses `script-src 'self' 'nonce-...' 'strict-dynamic'` so further bundles loaded by the entry script inherit trust without `'unsafe-inline'`. Routes that emit their own inline scripts (currently just the OAuth callback at `/api/admin/scheduled-exports/credentials/oauth/[provider]/callback`) read `request.headers.get("x-nonce")` and stamp it on their `<script>` tag.
