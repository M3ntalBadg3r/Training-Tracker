...
src/
  app/
    ...
    reports/      # Index page + 11 reports: by-product-type, by-function, expiring-soon, last-12-months, learner-scorecard (Learner Achievement Scorecard — one row per learner, client-side aggregation over /api/students + /api/reports/training-records + /api/reports/trained-not-certified; active cert/accred/ILT/OLX counts, renewing-soon, lapsed, cert gaps, last achievement, top-achievers leaderboard, active-only by default with include-expired toggle), trained-not-certified, coverage, comparison (theatre/region/country side-by-side matrix + chart, client-side over training-records + students), catalogue-health, program-compliance-trend, renewal-forecast
