    ...
    content: (
      <>
        <p>
          ...
          or PDF, and a PDF export can include the report&rsquo;s metric cards
          and charts as well as its data.
        </p>
        ...
        <ul>
          ...
          <li><strong>Include charts &amp; metrics in PDF</strong> — the Export menu carries an <strong>Include charts &amp; metrics in PDF</strong>{" "}tickbox (on by default; your choice is remembered on this device). With it ticked a PDF export opens with the report&rsquo;s KPI metric cards, then its charts, then the data table. The charts keep the layout they have on the page: two charts side by side on screen print side by side and in the same proportions, and a chart spanning the page keeps the full width — so if a narrow browser window has stacked the charts, the PDF stacks them too. The metric cards are drawn as text rather than photographed, so the numbers stay sharp at any zoom. It applies to PDF only: CSV is plain text and cannot hold a picture, and the Excel writer cannot embed one, so those two exports are unaffected. Everything captured reflects whatever filters, grouping and dates are in force when you export, and a panel currently showing no data is simply left out.</li>
