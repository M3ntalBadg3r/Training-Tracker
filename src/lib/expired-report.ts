...
/** One detail row — the same shape the training-records route returns. */
...
// ─── Deduped record fetch (identical logic to the training-records route) ───────
...
async function fetchDedupedRecords(companyFilter: number[] | null): Promise<ExpiredRow[]> {
...
export async function computeExpiredReport(input: ExpiredReportInput): Promise<ExpiredReportResult> {
  const records = await fetchDedupedRecords(input.companyIds);
