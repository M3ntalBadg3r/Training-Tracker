 ...
 * `/api/reports/renewal-forecast` and the public (API-key) route at
 * `/api/public/v1/reports/renewal-forecast` are both thin wrappers over this
 * one implementation and cannot drift — the same arrangement
 * `lib/program-report.ts` already has for the per-program dashboards.
 */

/**
 * The scope label for a geography selection (narrowest wins: country → region
 * → theatre). Exported so the routes can label their fail-closed empty payload
 * with the same string the computed one would have carried, rather than each
 * keeping its own copy of the rule.
 */
export function renewalForecastScopeLabel(
  countryParam: string,
  regionParam: string,
  theatreParam: string,
): string {
  if (countryParam) return `Country: ${countryParam}`;
  if (regionParam) return `Region: ${regionParam}`;
  if (theatreParam) return `Theatre: ${theatreParam}`;
  return "All theatres";
}

export async function computeRenewalForecast(
