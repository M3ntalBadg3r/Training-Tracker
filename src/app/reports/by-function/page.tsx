...
import { Award, ShieldCheck, GraduationCap, CircleCheck } from "lucide-react";
...
function ByFunctionPageInner() {
  ...
  const dataUrl = companyScope.loading
    ? null
    : withCompany(`/api/reports/by-function?${buildParams({}).toString()}`, companyScope.selected);
  const { data, loading } = useFetchJson<ByFunctionResponse>(dataUrl);
      ...
      <section className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
        <ExportableChart className="lg:col-span-2 bg-white rounded-lg border border-gray-200 p-5">
          <div className="flex items-center justify-between mb-4 gap-3 flex-wrap">
            <h3 className="text-base font-semibold text-gray-900">{countPeople ? "People by Function" : "Records by Function"}</h3>
            {filterFunction && (
