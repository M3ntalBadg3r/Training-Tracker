...
import { History, AlertCircle, AlertTriangle, Ban } from "lucide-react";
...
function LegacyGapPageInner() {
  ...
  const dataUrl = companyScope.loading
    ? null
    : withCompany(`/api/reports/legacy-gap?${buildParams({}).toString()}`, companyScope.selected);
  const { data, loading } = useFetchJson<LegacyGapResponse>(dataUrl);
  ...
  return (
    <div>
      ...
      <FilterBar>
        <FilterBar.Row>
          <SearchInput value={search} onChange={setSearch} placeholder="Search by name, email, or training…" />
          <GeoScopeFilter value={geo} onChange={setGeo} />
        </FilterBar.Row>
        <FilterBar.Row>
          <select value={filterWindow} onChange={(e) => setFilterWindow(e.target.value)} className={SELECT_CLASS}>
