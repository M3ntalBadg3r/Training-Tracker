...
export async function GET(request: NextRequest) {
  ...
  const now = new Date();
  const theatres = await listTheatres(companyFilter);

  const allTitles = extractTitles(programData);
  const emailSets = await getEmailSetsByTitle(allTitles, now, { companyIds: companyFilter });
  const byTitleAndTheatre = await getEmailSetsByTitleAndTheatre(allTitles, now, companyFilter);
